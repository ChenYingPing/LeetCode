...
class Hundred: UIViewController
{
    ...
    func sumNumbers(root: TreeNode?) -> Int {
        var arr: [Int] = []
        sumNumbersting(arr: &arr,root: root ,sum:"")
        var res = 0
        for num in arr {
            res += num
        }
    ...
    func sumNumbersting( arr: inout [Int], root: TreeNode?, sum: String) {
        if root == nil { return }
        var sum = sum
        sum += String(root!.val)
        if root?.left != nil {
            sumNumbersting(arr: &arr, root: root?.left, sum: sum)
        }
        if root?.right != nil {
            sumNumbersting(arr: &arr, root:root?.right, sum: sum)
        }
    ...
    func solve( board: inout [[Character]]) {
        ...
        for i in 0 ..< row {  // 遍历数组第一行的数据
            check(vec: &board,i: i,j: 0,row: row,col: col)
            if(col>1) {  // 遍历数组最后一列的数据
                check(vec: &board,i: i,j: col-1,row: row,col: col)
            }
        }
        for j in 0 ..< col {  // 遍历数组第一列的数组
            check(vec: &board,i: 0,j: j,row: row,col: col)
            if(row > 1) { // 遍历数组最后一行的数据
                check(vec: &board,i: row-1,j: j,row: row,col: col)
            }
        }
        for i in 0 ..< row {
            for j in 0 ..< col {
    ...
    func check( vec: inout [[Character]], i: Int, j: Int, row: Int, col: Int){
        if(vec[i][j] == Character.init("O")){  // 如果当前字符为O，则把它改为1，并且寻找它边上为O的字符也改为1，一直递归到找不到字符为O的位置。
            ...
            if(i > 1) {
                check(vec: &vec,i: i-1,j: j,row: row,col: col)
            }
            if(j > 1) {
                check(vec: &vec,i: i,j: j-1,row: row,col: col)
            }
            if(i+1 < row) {
                check(vec: &vec,i: i+1,j: j,row: row,col: col)
            }
    ...
    func candy(ratings: [Int]) -> Int {
        ...
        var arr = Array.init(repeating: 1, count: ratings.count)
        for i in 1 ..< ratings.count {
            if ratings[i] > ratings[i-1] {  // 后面的比之前的大
                arr[i] = arr[i-1] + 1
            }
        }
        for i in ratings.count-1 ... 1 {
...
//        return ret
        ...
        var bucketsMIN = Array.init(repeating: Int.max, count: nums.count - 1)
        var bucketsMAX = Array.init(repeating: Int.min, count: nums.count - 1)
        for i in nums {
...
//            result = result * 26 + (char - Character("A") + 1)
    ...
    func calculateMinimumHP(dungeon: [[Int]]) -> Int {
        ...
        let colsArr = Array.init(repeating: 0, count: cols)
        var dp = Array.init(repeating: colsArr, count: rows)
